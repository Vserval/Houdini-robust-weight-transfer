...
Input 2: (任意) 保護ウェイト参照（boneCapture を unpack した状態）
         Input 2 に入っているウェイト列は転写で上書きせず、Input 2 の値を優先してパススルーします。
...
#region agent log
_DBG_LOG_PATH = r"c:\Users\User\Desktop\Houdini\Python\ウェイトの転写\debug-272d83.log"
_DBG_SESSION_ID = "272d83"
def _dbg_log(location, message, data=None, runId="pre-fix", hypothesisId=""):
    try:
        payload = {
...
def get_capture_weights_dense(geo, prefix="boneCapture"):
    ...
    #region agent log
    _dbg_log(
        location="ウェイト転送_SOP.py:get_capture_weights_dense:entry",
        message="capture attrib presence",
        data={
            "prefix": prefix,
            "pt_idx": idx_attrib is not None,
            "pt_data": data_attrib is not None,
            "vtx_idx": (geo.findVertexAttrib(idx_name) is not None) if hasattr(geo, "findVertexAttrib") else None,
            "vtx_data": (geo.findVertexAttrib(data_name) is not None) if hasattr(geo, "findVertexAttrib") else None,
            "pt_count": int(geo.pointCount()) if hasattr(geo, "pointCount") else None,
            "prim_count": int(geo.primCount()) if hasattr(geo, "primCount") else None,
        },
        hypothesisId="A|B|C",
    )
    #endregion
    ...
    if idx_attrib is None or data_attrib is None:
        #region agent log
        _dbg_log(
            location="ウェイト転送_SOP.py:get_capture_weights_dense:return_none",
            message="missing boneCapture_index/data on both point and vertex",
            data={"prefix": prefix},
            hypothesisId="A|B|C",
        )
        #endregion
...
def set_capture_weights_from_dense(
    geo,
    W,
    prefix="boneCapture",
    bone_names=None,
    ...
    if is_vertex:
        ...
        n_elems = n_pts
        W_exp = W
    W_mask = np.where(W_exp > float(force_threshold), W_exp, 0.0).astype(np.float32)

    if force_include_cols is not None:
        force_include_cols = np.asarray(force_include_cols, dtype=bool)
        if force_include_cols.ndim != 1 or force_include_cols.shape[0] != n_bones:
            force_include_cols = None

    # 通常: 上位 slot_size を取り、選択分を正規化
    if force_include_cols is None or not np.any(force_include_cols):
        ...
        s = np.where(s < float(force_threshold), 1.0, s)
        topk_vals = (topk_vals / s).astype(np.float32)
        zero_mask = topk_vals < float(force_threshold)
        ...
        # 保護列はスロットに残し、残りを上位から埋める。
        # 正規化は「保護スロットは固定、非保護スロットだけを (1-保護合計) に合わせてスケール」。
        forced_cols = np.where(force_include_cols)[0].astype(np.int32)
        topk_idx = np.full((n_elems, slot_size), -1, dtype=np.int32)
        topk_vals = np.zeros((n_elems, slot_size), dtype=np.float32)
        for i in range(n_elems):
            w_row = W_mask[i, :]
            chosen = []

            fw = w_row[forced_cols]
            if fw.size:
                act = fw > float(force_threshold)
                if np.any(act):
                    cols = forced_cols[act]
                    vals = fw[act]
                    order = np.argsort(vals)[::-1]
                    cols = cols[order]
                    if cols.size > slot_size:
                        cols = cols[:slot_size]
                    chosen.extend([int(x) for x in cols])

            remaining = slot_size - len(chosen)
            if remaining > 0:
                wr = w_row.copy()
                if chosen:
                    wr[np.asarray(chosen, dtype=np.int32)] = -1.0
                if remaining == 1:
                    j = int(np.argmax(wr))
                    if float(wr[j]) > float(force_threshold):
                        chosen.append(j)
                else:
                    if n_bones <= remaining:
                        cand = np.arange(n_bones, dtype=np.int32)
                    else:
                        cand = np.argpartition(wr, -remaining)[-remaining:].astype(np.int32)
                    cand = cand[np.argsort(wr[cand])[::-1]]
                    for j in cand:
                        if len(chosen) >= slot_size:
                            break
                        if float(wr[int(j)]) <= float(force_threshold):
                            continue
                        chosen.append(int(j))
            ...
            m = min(len(chosen), slot_size)
            idxs = np.asarray(chosen[:m], dtype=np.int32)
            vals = w_row[idxs].astype(np.float32, copy=False)
            topk_idx[i, :m] = idxs
            topk_vals[i, :m] = vals

            prot_mask = force_include_cols[idxs]
            prot_sum = float(np.sum(vals[prot_mask])) if np.any(prot_mask) else 0.0
            prot_sum = max(0.0, min(1.0, prot_sum))
            free_mask = ~prot_mask
            free_sum = float(np.sum(vals[free_mask])) if np.any(free_mask) else 0.0
            if prot_sum >= 1.0 - 1e-9:
                if prot_sum > float(force_threshold):
                    vals2 = np.zeros_like(vals)
                    vals2[prot_mask] = vals[prot_mask] / prot_sum
                    topk_vals[i, :m] = vals2
            else:
                if free_sum > float(force_threshold):
                    scale = (1.0 - prot_sum) / free_sum
                    vals2 = vals.copy()
                    vals2[free_mask] = vals2[free_mask] * float(scale)
                    topk_vals[i, :m] = vals2
                else:
                    if prot_sum > float(force_threshold):
                        vals2 = np.zeros_like(vals)
...
def _bone_name_variants(name):
    ...
    out = {s}
    if "__" in s:
        out.add(s.split("__")[-1])
    if ":" in s:
        out.add(s.split(":")[-1])
    out |= {x.lower() for x in list(out)}
    return out
...
def remap_weights_to_skeleton_no_normalize(W_old, old_paths, new_paths, progress_cb=None):
    """
    remap_weights_to_skeleton の「正規化しない版」。
    ボーン列対応を揃えるだけで、行和は変更しない。
    """
    prog = progress_cb or (lambda *a: None)
    n_pts = W_old.shape[0]
    n_new = len(new_paths)
    if n_new == 0:
        prog("  [ボーンマップ] 新しい pCaptPath が 0 件のためスキップ（no_norm）")
        return W_old
    new_names = [_extract_bone_name(p) for p in new_paths]
    old_to_new = np.full(W_old.shape[1], -1, dtype=np.int32)
    matched = 0
    for old_idx in range(W_old.shape[1]):
        ...
        for new_idx, nn in enumerate(new_names):
            if nn == old_name:
                ...
                matched += 1
                break
    prog("  ボーンマップ(no_norm): {} / {} 一致（新しいスケルトン {} ボーン）".format(matched, W_old.shape[1], n_new))
    W_new = np.zeros((n_pts, n_new), dtype=np.float32)
    for old_idx in range(W_old.shape[1]):
        new_idx = old_to_new[old_idx]
        if new_idx >= 0:
...
def _merge_protected_weights(W_transferred, W_protect_src, protect_cols, eps=1e-8, progress_cb=None):
    """
    protect_cols の列は W_protect_src を固定（上書き禁止）し、それ以外の列は W_transferred を
    (1 - sum(protected)) に合わせてスケールして行和が 1 になるように合成する。
    """
    prog = progress_cb or (lambda *a: None)
    Wt = np.asarray(W_transferred, dtype=np.float64)
    Wp = np.asarray(W_protect_src, dtype=np.float64)
    protect_cols = np.asarray(protect_cols, dtype=bool)
    if Wt.shape != Wp.shape:
        raise ValueError("W shape mismatch: transferred={} protect_src={}".format(Wt.shape, Wp.shape))
    if protect_cols.ndim != 1 or protect_cols.shape[0] != Wt.shape[1]:
        raise ValueError("protect_cols mismatch: {} vs bones={}".format(protect_cols.shape, Wt.shape[1]))
    if not np.any(protect_cols):
        return np.clip(Wt, 0.0, 1.0).astype(np.float32)

    out = np.zeros_like(Wt)
    locked = np.maximum(Wp[:, protect_cols], 0.0)
    locked_sum = np.sum(locked, axis=1)  # (n,)
    locked_sum_clamped = np.clip(locked_sum, 0.0, 1.0)
    out[:, protect_cols] = locked

    free = np.maximum(Wt, 0.0)
    free[:, protect_cols] = 0.0
    free_sum = np.sum(free, axis=1)  # (n,)
    remaining = 1.0 - locked_sum_clamped

    has_free = free_sum > eps
    if np.any(has_free):
        scale = remaining[has_free] / free_sum[has_free]
        out[has_free, :] += free[has_free, :] * scale[:, None]

    # free が全く無いが remaining がある行は、locked を 1 に正規化（この場合 “固定値を絶対維持” は不可能）
    need_norm_locked = (~has_free) & (locked_sum > eps)
    if np.any(need_norm_locked):
        prog("  [警告] 非保護側のウェイトが 0 の頂点があるため、保護列のみで正規化します")
        out[need_norm_locked, :] = 0.0
        prot_idx = np.where(protect_cols)[0]
        out[np.ix_(need_norm_locked, prot_idx)] = locked[need_norm_locked, :] / locked_sum[need_norm_locked][:, None]
...
def run_robust_weight_transfer(
    ...
    if len(inputs) >= 3 and inputs[2] is not None:
        try:
            ref_geo = inputs[2].geometry().freeze()
        except Exception:
            try:
    ...
    protect_cols_for_pack = None
    if ref_geo is not None:
        prog("  Input 2 接続: 保護ウェイトを優先してパススルーします")
        #region agent log
        try:
            pt_attrs = []
            try:
                pt_attrs = [a.name() for a in ref_geo.pointAttribs()]
            except Exception:
                pt_attrs = []
            vtx_attrs = []
            try:
                vtx_attrs = [a.name() for a in ref_geo.vertexAttribs()] if hasattr(ref_geo, "vertexAttribs") else []
            except Exception:
                vtx_attrs = []
            glob_attrs = []
            try:
                glob_attrs = [a.name() for a in ref_geo.globalAttribs()] if hasattr(ref_geo, "globalAttribs") else []
            except Exception:
                glob_attrs = []
            _dbg_log(
                location="ウェイト転送_SOP.py:run_robust_weight_transfer:input2_summary",
                message="Input2 geo summary",
                data={
                    "pt_count": int(ref_geo.pointCount()) if hasattr(ref_geo, "pointCount") else None,
                    "prim_count": int(ref_geo.primCount()) if hasattr(ref_geo, "primCount") else None,
                    "has_pt_boneCapture_index": ref_geo.findPointAttrib("boneCapture_index") is not None,
                    "has_pt_boneCapture_data": ref_geo.findPointAttrib("boneCapture_data") is not None,
                    "has_vtx_boneCapture_index": (ref_geo.findVertexAttrib("boneCapture_index") is not None) if hasattr(ref_geo, "findVertexAttrib") else None,
                    "has_vtx_boneCapture_data": (ref_geo.findVertexAttrib("boneCapture_data") is not None) if hasattr(ref_geo, "findVertexAttrib") else None,
                    "pt_attrs_sample": pt_attrs[:40],
                    "vtx_attrs_sample": vtx_attrs[:40],
                    "global_attrs_sample": glob_attrs[:60],
                },
                hypothesisId="A|B|C",
            )
        except Exception:
    ...
    # ターゲット側の「元ウェイト」と「pCaptPath」を先に取得（保護・出力骨体系に使う）
    tgt_W0, _, tgt_slot_size, tgt_capture_class = get_capture_weights_dense(tgt_geo)
    tgt_meta = _get_capture_metadata(tgt_geo, "boneCapture")
    tgt_paths = tgt_meta["paths"]
    #region agent log
    _dbg_log(
        location="ウェイト転送_SOP.py:run_robust_weight_transfer:target_capture_meta",
        message="target capture meta",
        data={
            "tgt_paths_len": int(len(tgt_paths)),
            "tgt_W0": None if tgt_W0 is None else [int(tgt_W0.shape[0]), int(tgt_W0.shape[1])],
            "tgt_capture_class": str(tgt_capture_class),
            "tgt_slot_size": int(tgt_slot_size) if tgt_slot_size is not None else None,
        },
        hypothesisId="E2_fix",
    )
    #endregion
    ...
    # ここから先の出力骨体系は「ターゲットのpCaptPath（存在する場合）」を優先する。
    # Input2(スケルトン)のnameや、Input1の既存ウェイトはターゲット骨体系に乗っているため。
    out_meta_src = src_geo
    out_slot_size = slot_size
    out_capture_class = capture_class
    if tgt_paths:
        prog("[5.2/6] 出力ボーン体系をターゲットへ合わせる（pCaptPathベース）...")
        src_meta = _get_capture_metadata(src_geo, "boneCapture")
        src_paths_all = src_meta["paths"]
        if src_paths_all:
            W2 = remap_weights_to_skeleton(W2, src_paths_all[: W2.shape[1]], tgt_paths, progress_cb=prog)
            out_meta_src = tgt_geo
            out_slot_size = tgt_slot_size if tgt_slot_size is not None else slot_size
            out_capture_class = tgt_capture_class if tgt_capture_class else capture_class
        #region agent log
        _dbg_log(
            location="ウェイト転送_SOP.py:run_robust_weight_transfer:remap_to_target_paths",
            message="remapped transfer weights to target skeleton",
            data={"W2_shape": [int(W2.shape[0]), int(W2.shape[1])], "out_meta_src": "tgt_geo"},
            hypothesisId="E2_fix",
        )
        #endregion

    # Input 2: 保護（優先パススルー／マスク）
    # - Cd マスクモード: Input2 にメッシュ＋Cd が来た場合、その Cd を 0〜1 のマスクとして
    #   「W2（転写ウェイト）と Input1 の元ウェイト（tgt_W0）」を補間する
    # - boneCapture モード: Input2 に boneCapture がある場合、そのウェイト列を保護用として使用
    # - スケルトン名モード: Input2 がスケルトンのみ（boneCapture無し）の場合、name から保護ボーンを決定し、
    #   Input1（ターゲット）の元ウェイトを優先パススルーする
    if ref_geo is not None:
        prog("[5.5/6] Input 2 から保護情報を取得...")

        # まず Cd マスクモードを試す: Input2 がターゲットと同じポイント数を持ち、Cd がある場合
        did_cd_mask = False
        if tgt_W0 is not None and tgt_W0.shape == W2.shape and tgt_capture_class == "point":
            try:
                if ref_geo.pointCount() == tgt_geo.pointCount() and ref_geo.findPointAttrib("Cd") is not None:
                    try:
                        # 可能なら高速APIで取得
                        cd_flat = np.frombuffer(ref_geo.pointFloatAttribValuesAsString("Cd"), dtype=np.float32)
                        cd = cd_flat.reshape(-1, 3)[: ref_geo.pointCount()]
                    except Exception:
                        cd_vals = ref_geo.pointFloatAttribValues("Cd")
                        cd = np.array(cd_vals, dtype=np.float32).reshape(-1, 3)[: ref_geo.pointCount()]
                    # グレースケール（単純平均）をマスクとする
                    m = np.clip(cd.mean(axis=1), 0.0, 1.0).astype(np.float32)
                    if m.shape[0] == tgt_geo.pointCount():
                        m_col = m.reshape(-1, 1)
                        # Cd=0 → 完全転写（W2）、Cd=1 → 完全保護（tgt_W0）
                        W2 = (1.0 - m_col) * W2 + m_col * tgt_W0
                        prog("  Input 2: Cd をマスクとして W2 と元ウェイトをブレンドしました")
                        did_cd_mask = True
            except Exception:
                # Cd マスクがうまく取得できなければ後続のモードにフォールバック
                did_cd_mask = False

        # Cd マスクを適用できなかった場合のみ、従来の boneCapture / スケルトン名モードを使う
        if not did_cd_mask:
            src_paths_all = _get_capture_metadata(src_geo, "boneCapture")["paths"]
            src_paths = src_paths_all[: W2.shape[1]] if src_paths_all else []

            ref_W, _, _, ref_class = get_capture_weights_dense(ref_geo)
            if ref_W is not None and ref_W.shape[1] > 0:
                prog("  Input 2: ウェイトを検出（優先パススルー）")
                ref_paths = _get_capture_metadata(ref_geo, "boneCapture")["paths"]
                if src_paths and ref_paths:
                    ref_W = remap_weights_to_skeleton_no_normalize(ref_W, ref_paths, src_paths, progress_cb=prog)
                if ref_class == "point" and ref_W.shape[0] == tgt_geo.pointCount() and ref_W.shape[1] == W2.shape[1]:
                    protect_cols = np.max(ref_W, axis=0) > 1e-6
                    n_prot = int(np.sum(protect_cols))
                    #region agent log
                    _dbg_log(
                        location="ウェイト転送_SOP.py:run_robust_weight_transfer:protect_from_input2_weights",
                        message="protect cols from Input2 weights",
                        data={"n_protect_cols": n_prot, "ref_pts": int(ref_W.shape[0]), "tgt_pts": int(tgt_geo.pointCount())},
                        hypothesisId="D",
                    )
                    #endregion
                    if n_prot > 0:
                        prog("  保護ボーン: {} 列（Input 2 の非ゼロ列）".format(n_prot))
                        W2 = _merge_protected_weights(W2, ref_W, protect_cols, progress_cb=prog)
                        protect_cols_for_pack = protect_cols
                        #region agent log
                        try:
                            err = float(np.max(np.abs(W2[:, protect_cols] - ref_W[:, protect_cols]))) if np.any(protect_cols) else 0.0
                        except Exception:
                            err = None
                        _dbg_log(
                            location="ウェイト転送_SOP.py:run_robust_weight_transfer:protect_verify_input2_weights",
                            message="verify protected cols unchanged (Input2 weights)",
                            data={"max_abs_error": err, "n_protect_cols": n_prot, "W2_shape": [int(W2.shape[0]), int(W2.shape[1])]},
                            hypothesisId="V",
                        )
                        #endregion
                    else:
                        prog("  [警告] Input 2 側で非ゼロ列が見つかりません -> 保護スキップ")
                else:
                    prog("  [警告] Input 2 のウェイトがターゲット点数と一致しないためスキップ（ref_pts={}, tgt_pts={} / class={}）".format(
                        ref_W.shape[0], tgt_geo.pointCount(), ref_class
                    ))
            else:
                prog("  Input 2: boneCapture無し（スケルトン）として保護ボーン名を使用")
            # 保護ボーン名: Input2 の point string 'name' を優先
            protect_names = []
            try:
                if ref_geo.findPointAttrib("name") is not None:
                    protect_names = [str(x) for x in ref_geo.pointStringAttribValues("name")]
            except Exception:
                protect_names = []
            protect_names = [_extract_bone_name(n) for n in protect_names if str(n).strip()]
            #region agent log
            _dbg_log(
                location="ウェイト転送_SOP.py:run_robust_weight_transfer:skeleton_protect_input",
                message="skeleton protect inputs",
                data={
                    "protect_names_len": int(len(protect_names)),
                    "protect_names_sample": protect_names[:20],
                    "src_paths_len": int(len(src_paths)),
                    "src_paths_sample": [str(x) for x in src_paths[:12]],
                },
                hypothesisId="E1|E2",
            )
            #endregion
            # 保護列の決定先は「ターゲット骨体系（tgt_paths）」を優先する
            protect_base_paths = tgt_paths if tgt_paths else src_paths
            if protect_names and protect_base_paths:
                out_names = [_extract_bone_name(p) for p in protect_base_paths]
                v2i = {}
                for i, n in enumerate(out_names):
                    for v in _bone_name_variants(n):
                        if v and v not in v2i:
                            v2i[v] = i
                protect_cols = np.zeros(W2.shape[1], dtype=bool)
                unmatched = 0
                for bn in protect_names:
                    idx = None
                    for v in _bone_name_variants(bn):
                        idx = v2i.get(v, None)
                        if idx is not None:
                            break
                    if idx is not None and 0 <= idx < protect_cols.shape[0]:
                        protect_cols[idx] = True
                    else:
                        unmatched += 1
                n_prot = int(np.sum(protect_cols))
                #region agent log
                _dbg_log(
                    location="ウェイト転送_SOP.py:run_robust_weight_transfer:skeleton_protect_match",
                    message="skeleton->pCaptPath match result",
                    data={
                        "n_protect_cols": n_prot,
                        "unmatched_names": int(unmatched),
                        "out_names_sample": out_names[:20],
                    },
                    hypothesisId="E2",
                )
                #endregion

                # パススルー元ウェイト: Input1（ターゲット）の“元”ウェイト
                tgt_W = tgt_W0
                tgt_class = tgt_capture_class
                #region agent log
                _dbg_log(
                    location="ウェイト転送_SOP.py:run_robust_weight_transfer:skeleton_protect_target",
                    message="target base weight availability",
                    data={
                        "tgt_W": None if tgt_W is None else [int(tgt_W.shape[0]), int(tgt_W.shape[1])],
                        "tgt_class": tgt_class,
                        "tgt_paths_len": int(len(tgt_paths)),
                        "tgt_paths_sample": [str(x) for x in tgt_paths[:12]] if tgt_paths else [],
                    },
                    hypothesisId="E3|E4",
                )
                #endregion
                if tgt_W is None or tgt_W.shape[1] == 0 or tgt_class != "point" or tgt_W.shape[0] != tgt_geo.pointCount():
                    prog("  [警告] Input 1 の元ウェイトが取得できないため、スケルトン保護をスキップ")
                else:
                    # W2 は既にターゲット骨体系にリマップ済み（tgt_paths）なので、tgt_W はそのままでOK
                    if tgt_W.shape[1] != W2.shape[1]:
                        prog("  [警告] ターゲット元ウェイトの列数が一致しないためスキップ（{} vs {}）".format(tgt_W.shape[1], W2.shape[1]))
                    elif n_prot > 0:
                        #region agent log
                        _dbg_log(
                            location="ウェイト転送_SOP.py:run_robust_weight_transfer:protect_from_skeleton_names",
                            message="protect cols from skeleton names",
                            data={"n_protect_cols": n_prot, "protect_names_sample": protect_names[:20]},
                            hypothesisId="A",
                        )
                        #endregion
                        prog("  保護ボーン: {} 列（Input 2 の name から）".format(n_prot))
                        W2 = _merge_protected_weights(W2, tgt_W, protect_cols, progress_cb=prog)
                        protect_cols_for_pack = protect_cols
                        #region agent log
                        try:
                            err = float(np.max(np.abs(W2[:, protect_cols] - tgt_W[:, protect_cols]))) if np.any(protect_cols) else 0.0
                        except Exception:
                            err = None
                        _dbg_log(
                            location="ウェイト転送_SOP.py:run_robust_weight_transfer:protect_verify_skeleton_names",
                            message="verify protected cols unchanged (skeleton names -> target base weights)",
                            data={"max_abs_error": err, "n_protect_cols": n_prot, "W2_shape": [int(W2.shape[0]), int(W2.shape[1])]},
                            hypothesisId="V",
                        )
                        #endregion
                    else:
                        prog("  [警告] Input 2 の name が pCaptPath に一致せず、保護列を作れませんでした -> スキップ")
            else:
                prog("  [警告] Input 2 の name または src の pCaptPath が無いため保護できません -> スキップ")
    ...
    ensure_capture_attribs(out_geo, "boneCapture", W2.shape[1], num_slots=out_slot_size, capture_class=out_capture_class)
    set_capture_weights_from_dense(
        out_geo,
        W2,
        "boneCapture",
        bone_names,
        ...
        capture_class=out_capture_class,
        force_include_cols=protect_cols_for_pack,
    )
...
if not globals().get("__rwt_gui_mode__"):
    ...
    if len(inputs) >= 2:
        # Input 0: 体（Unpack済み）, Input 1: 衣装, Input 2: 任意・保護ウェイト参照（Unpack済みジオメトリ）
        ...
        raise hou.NodeError("Input 0 (source body) と Input 1 (target clothing) を接続してください。Input 2 は任意（保護ウェイト参照）")
