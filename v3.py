...
def run_robust_weight_transfer(
    ...
    # Input 2: 保護（優先パススルー）
    # - Input2 に boneCapture がある場合: Input2 のウェイトを優先（点数一致が必要）
    # - Input2 がスケルトンのみ（boneCapture無し）の場合: Input2 の bone 名リストを「保護ボーン」として扱い、
    #   Input1（ターゲット）にもともと乗っているウェイトを優先してパススルーする
    if ref_geo is not None:
